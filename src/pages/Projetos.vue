      ...
      <div class="modal" :class="{ show: modalAberto }" @click="fecharModal">
        <div class="modal-content" @click.stop>
          ...
          <div v-if="projetoSelecionado" class="carousel">
            ...
            <img 
              :src="projetoSelecionado.imagens[imagemAtualIndex]" 
          ...
          <div class="modal-button">
            ...
            <a 
              v-if="projetoSelecionado?.codigoUrl"
              :href="projetoSelecionado.codigoUrl"
              target="_blank"
              class="btn"
            >
...
const fecharDropdown = () => {
  setTimeout(() => (dropdownAberto.value = false), 200)
}
...
const prevImagem = () => {
  const total = projetoSelecionado.value?.imagens.length || 1
  imagemAtualIndex.value =
    (imagemAtualIndex.value - 1 + total) % total
}

const nextImagem = () => {
  const total = projetoSelecionado.value?.imagens.length || 1
  imagemAtualIndex.value =
    (imagemAtualIndex.value + 1) % total
}
...
onMounted(() => {
  ...
  categoriasUnicas.value = [
    ...new Set(projetos.value.map((p) => p.categoria)),
  ]
})
</script>
...
button.btn-ver-mais:hover{
    font-size: 1.2rem;
    transition: 0.60s ease, color 0.60s ease;
}
button.btn-ver-mais:hover{
